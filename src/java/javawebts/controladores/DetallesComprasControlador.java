...
import javawebts.modelos.DetallesCompras;
...
public class DetallesComprasControlador {
            ...
            try {
                ...
                try (PreparedStatement ps = Conexion.getConn().prepareStatement(sql)) {
                    ...
                    while (rs.next()) {
                        ...
                        tabla += "<tr>"
                                //   + "<td>" + rs.getInt("id_detallecompra") + "</td>"
                                //  + "<td>" + rs.getInt("id_producto") + "</td>"

                                + "<td class='centrado'>" + df.format(cantidad) + "</td>"
                                + "<td>" + rs.getString("nombre_marca") + " " +rs.getString("nombre_producto") + " " + "</td>"
                                + "<td>" + df.format(rs.getInt("costo_producto")) + "</td>"
                                + "<td>" + df.format(totalexentas) + "</td>"
                                + "<td>" + df.format(totaliva5) + "</td>"
                                + "<td>" + df.format(totaliva10) + "</td>"
                                + "<td class='centrado'> "
                                + "<button id='editarCompra' onclick='editarLinea(" + rs.getString("id_detallecompra") + ")'"
                                + " type='button' class='btn btn-primary btn-sm botonEditarCompra'><span class='glyphicon glyphicon-remove'>"
                                + "</span></button></td>"
                                + "</tr>";
    ...
    public static boolean agregar(DetallesCompras detallecompra) throws SQLException {
        ...
        if (Conexion.conectar()) {
            String sql = "insert into detallescompras "
                    + "(id_compra,id_producto,cantidad_productocompra,total_detallecompra) "
                    + "values (?,?,?,?)";
            ...
            try (PreparedStatement ps = Conexion.getConn().prepareStatement(sql)) {
                ...
                ps.setInt(4, detallecompra.getTotal_detallecompra());
                ps.executeUpdate();
                ps.close();
    ...
    public static boolean modificar(DetallesCompras detallecompra) throws SQLException {
        ...
        if (Conexion.conectar()) {
            ...
            try (PreparedStatement ps = Conexion.getConn().prepareStatement(sql)) {
                ...
                ps.setInt(3, detallecompra.getCantidad_productocompra());
                ps.setInt(4, detallecompra.getTotal_detallecompra());
                ps.setInt(5, detallecompra.getId_detallecompra());
                ps.executeUpdate();
                ps.close();
                Conexion.getConn().commit();
    ...
    public static boolean eliminar(DetallesCompras detallecompra) throws SQLException {
        ...
        if (Conexion.conectar()) {
            ...
            System.out.println(" operacion realizafa  "+ sql );
            try (PreparedStatement ps = Conexion.getConn().prepareStatement(sql)) {
    ...
    public static boolean eliminarcompra(DetallesCompras detallecompra) throws SQLException {
        boolean valor = false;
        if (Conexion.conectar()) {
            try {
                String sql = "select * from detallescompras dp "
                ...
                try (PreparedStatement ps = Conexion.getConn().prepareStatement(sql)) {
                    ...
                    while (rs.next()) {
                        ...
                        String sqli = "update stocks set cantidad_stock= cantidad_stock - "+ rs.getInt("cantidad_productocompra") + " where id_producto="+ rs.getInt("id_producto")+"";

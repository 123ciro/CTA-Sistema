...
import javawebts.modelos.Permisos;
...
public class PermisosControlador {
    ...
    public static Permisos buscarId(Permisos permiso) {
        if (Conexion.conectar()) {
            ...
            + "roles.id_rol=permisos.id_rol and formularios.id_formulario=permisos.id_formulario" +
            " and id_permiso='" + permiso.getId_permiso() + "'";
            try {
    ...
    public static String buscarOrdenPermiso(Permisos ordenpermiso, int pagina) {
        int offset = (pagina - 1) * Utiles.REGISTROS_PAGINA;
        String valor = "";
        if (Conexion.conectar()) {
            try {
                String sql = "select * from permisos where upper(orden_permiso) like '%" + ordenpermiso.getOrden_permiso() + "%'"
